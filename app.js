(() => {
      ...
      if (images.length) {
        ...
        for (const file of targets) {
          try {
            ...
            return; // ← handleAttachment 内なので合法
          }
        }
  ...
  try {
    ...
    // ✅ 1) まず圧縮を試す（iOSで失敗することがある）
    let blob = null;
    try {
      blob = await imageFileToCompressedBlob(file, 1280, 0.72); // ←あなたの関数
    } catch (_) {
      blob = null;
    }

    // ✅ 2) 圧縮がダメなら “元ファイル” をそのまま保存（最強安定）
    if (!blob) {
      blob = file; // File は Blob なのでそのままIDBへ入る
    }
    ...
    // ✅ 3) iOS対策：保存できたか即検証（ここでblobが取れないとiOSの制限）
    const verify = await PhotosDB.get(pid);
    if (!verify || !verify.blob) {
      throw new Error("IndexedDB write/read verification failed");
    }

    // ✅ プレビューは dataURL（iOSでObjectURLが不安定なことがある）
    const dataUrl = await blobToDataURL(verify.blob);

    // url保持しない（revoke不要）
    State.pendingPhotos.push({ id: pid, url: "", name, shotAt, shotTime });

    const img = document.createElement("img"); // ←これ必須
    img.src = dataUrl;
    img.alt = "attachment";
    grid.appendChild(img);
    ...
    console.error("photo attach failed:", e);
    Toast.show("写真の保存に失敗（iPhoneの制限/プライベートモード/容量）");
    return;
  }
}
