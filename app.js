(() => {
  ...
  const Storage = (() => {
    ...
    function normalizeDay(dateKey, raw) {
      ...
      const base = {
        ...
        summary: day.summary || "",
        noteshelf: {
          theme: day?.noteshelf?.theme || "dark",
          textColor: day?.noteshelf?.textColor || "#FFFFFF",
    ...
    function exportAll() {
      ...
      for (const k of Object.keys(localStorage)) {
        if (/^\d{4}-\d{2}-\d{2}$/.test(k)) out[k] = getDay(k);
      }
      return out;
    }

    function dayHasContent(dayOrDateKey) {
      const day = typeof dayOrDateKey === "string" ? getDay(dayOrDateKey) : dayOrDateKey || {};
      const hasEntries = Array.isArray(day.entries) && day.entries.length > 0;
      const hasSummary = !!String(day.summary || "").trim();
      return hasEntries || hasSummary;
    }
  ...
  const Calendar = (() => {
    ...
    function renderWeekBar(dateStr) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (Storage.dayHasContent(ymd)) btn.classList.add("hasEntry");
    ...
    function renderMonthCalendar() {
      ...
      for (let day = 1; day <= lastDate; day++) {
        ...
        if (Storage.dayHasContent(dayStr)) btn.classList.add("hasEntry");
    ...
    function attachSwipeHandlers(wrapEl, contentEl) {
      wrapEl.addEventListener(
        ...
        (e) => {
          ...
          swipeLock.currentX = wrapEl.classList.contains("open") ? SWIPE_OPEN_X : 0;
      ...
      const finish = async () => {
        ...
        wrapEl.dataset.justSwiped = "1";
        setTimeout(() => {
          delete wrapEl.dataset.justSwiped;
        }, 180);
      ...
      if (saved.entries && saved.entries.length) {
        for (const entry of saved.entries) {
          ...
          content.addEventListener("click", async () => {
            ...
            if (wrap.dataset.justSwiped === "1") return;
  ...
  const Input = (() => {
    ...
    async function handleAttachment(fileList) {
      ...
      if (images.length) {
        ...
        if (IS_SAFARI && images.length > 1) Toast.show("Safariでは写真は1回1枚が安定");
        ...
        for (const file of safeImages) {
          try {
            ...
            if (!verify || !verify.blob) throw new Error("IndexedDB write/read verification failed");
            ...
            if (/IndexedDB|database|quota/i.test(msg)) Toast.show("写真保存失敗：Safariの保存制限の可能性");
            else Toast.show("写真の保存に失敗");
            break;
          }
        }
      ...
      if (nonImages.length) {
        ...
        if (DOM.previewArea) {
          DOM.previewArea.innerHTML = `<div class="filePreview">📎 ${escapeHtml(State.pendingFileName)}</div>`;
        }
        setPreviewVisible(true);
        expand();
        Layout.scheduleUpdate();
    ...
    async function beginEditEntry(entry) {
      ...
      State.editingPrevPhotoIds = Array.isArray(entry.photos) ? entry.photos.map((p) => p.id) : [];
    ...
    function closePlusSheet() {
      ...
      if (ae && DOM.plusSheet && DOM.plusSheet.contains(ae) && typeof ae.blur === "function") {
        ae.blur();
      }
  ...
  const App = (() => {
    ...
    async function init() {
      ...
      if (ok === false) Toast.show("写真DBが使えない状態");
