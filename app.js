(() => {
  ...
  console.log("LOGDAY APP VERSION 2026-03-16-01");
alert("LOGDAY APP VERSION 2026-03-16-01");
  ...
  const Header = (() => {
    ...
    function renderWeekBar(dateStr) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (Storage.dayHasContent(saved)) btn.classList.add("hasEntry");
  ...
  const Entries = (() => {
    ...
    function attachSwipeHandlers(wrapEl, contentEl) {
      ...
      const finish = async () => {
        ...
        setTimeout(() => {
          if (wrapEl) delete wrapEl.dataset.justSwiped;
        }, 180);
