(() => {
  ...
  // ✅ 二重読み込み・再実行ガード（PWA/キャッシュ/復帰で同一JSが再評価されても安全）
  ...
  function applyTimeStepToUI(stepMin){
    timeStepMin = stepMin;
    if (logTimeEl) logTimeEl.step = String(stepMin * 60);
  }
  ...
  logInput?.addEventListener('beforeinput', () => {
    if (inputBar && !inputBar.classList.contains('expanded')) expandInputBar();
  });
  ...
  function closePlusSheet(){
    ...
    if (ae && plusSheet && plusSheet.contains(ae) && typeof ae.blur === 'function') ae.blur();
  ...
  if (plusBtn) plusBtn.onclick = openPlusSheet;
  if (plusSheetBackdrop) plusSheetBackdrop.onclick = closePlusSheet;
  if (actCancel) actCancel.onclick = closePlusSheet;

  if (actPhoto)  actPhoto.onclick  = () => { closePlusSheet(); photoEl?.click(); };
  if (actCamera) actCamera.onclick = () => { closePlusSheet(); cameraEl?.click(); };
  if (actFile)   actFile.onclick   = () => { closePlusSheet(); fileInputEl?.click(); };
  ...
  async function handleAttachment(input){
    ...
    if (nonImages.length) {
      ...
      if (previewArea) previewArea.innerHTML = `<div class="filePreview">📎 ${pendingFileName}</div>`;
      setPreviewVisible(true);
      expandInputBar();
      logInput?.focus?.();
    }
  ...
  async function beginEditEntry(entry){
    ...
    expandInputBar();
    if (logInput) logInput.value = entry.text || "";

    // time
    if (!entry.time) {
      timeMode = 'auto';
      selectedTime = '';
      if (logTimeEl) logTimeEl.value = '';
    } else {
      timeMode = 'set';
      selectedTime = entry.time;
      if (logTimeEl) logTimeEl.value = entry.time;
    }
    refreshTimeHint();
    ...
    saveBtn.onclick = async () => {
      ...
      try {
        ...
        if (timeMode === 'set') {
          ...
          selectedTime = time;
          if (logTimeEl) logTimeEl.value = time;
  ...
  function closeSettingsModal(){
    const ae = document.activeElement;
    if (ae && settingsModal && settingsModal.contains(ae) && typeof ae.blur === 'function') ae.blur();
  ...
  /* export/import */
  const exportBtn = byId('exportLogday');
  if (exportBtn) {
    exportBtn.onclick = () => {
      const out = {};
...
saveBtn.onclick = async () => {
  ...
  try {
    ...
    if (editingId) {
      // ✅ 編集：写真は「差し替えない限り維持」
      // ・編集開始時：元写真があれば pendingPhotos に入ってる
      // ・写真を選び直したら pendingPhotos[0].id が新しいIDになる
      // ・万一 pendingPhotos が空なら、editingPrevPhotoId を維持
      const nextPhotoId = hasPhoto
        ? (pendingPhotos[0]?.id || null)
        : (editingPrevPhotoId || null);
      ...
      // ✅ 旧写真の削除は「差し替えた時だけ」
      // prev があり、next もあり、かつIDが変わった場合のみ削除
      const replaced = !!prevId && !!nextId && (prevId !== nextId);
      if (replaced) {
        try { await idbDeletePhoto(prevId); } catch(e){ console.warn(e); }
      }
    ...
    pendingFileName = '';
    revokePendingPhotoUrls(); // preview用ObjectURLを破棄して pendingPhotos を空にする
